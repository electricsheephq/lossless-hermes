...
| **Wave-10** | embeddings/hybrid-search | Rerank-pack 510K-token budget (= `floor(MAX_TOKENS_PER_RERANK_CALL * 0.85)`). Previously sent ALL candidates' full content to rerank without enforcing the ~600K token cap, so a query with many large condensed summaries either (a) hit Voyage 400 and silently degraded to RRF (losing the +52.5pp paraphrastic lift), or (b) consumed the entire month's quota in one call. Pack candidates until cumulative would exceed the cap; surface `rerank_pack_truncated` + `rerank_packed_count` so the operator can warn. | `src/embeddings/hybrid-search.ts:326-360` | `src/lossless_hermes/embeddings/hybrid_search.py` |
| **Wave-11** | embeddings/hybrid-search | Skip individually-oversized candidates with `continue`, not `break`. Earlier TS code broke out of the loop when the first candidate was oversized, disabling rerank for the entire result set even though smaller later candidates would fit. Now a single 700K-token condensed summary no longer takes down the whole rerank — it's skipped and the loop continues packing smaller neighbors. The oversized candidate stays in `candidates` for the RRF backstop. | `src/embeddings/hybrid-search.ts:341-352` | `src/lossless_hermes/embeddings/hybrid_search.py` |
